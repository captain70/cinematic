...
const SearchedShows = () => {
	...
	const getShow = async (name) => {
		axios
			.get(
				`https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&query=${name}&page=1`
			)
			.then((response) => {
	...
	return (
		<div className=' grid grid-cols-5 ml-20 gap-y-5 mt-20 min-h-screen'>
			{searched.map((movie, id) => {
				return (
					<div key={id} className='w-40 h-60 group cursor-pointer perspective'>
