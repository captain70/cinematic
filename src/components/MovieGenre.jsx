...
const MovieGenre = ({ movie, setMovies, series, setSeries }) => {
	const fetchSubData = useCallback(
		(text) => {
			let url = `https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&query=${text}&page=1`;
					...
					<Menu.Items className='absolute left-0 mt-2 w-44 origin-top-right divide-y divide-gray-100 rounded-lg shadow-lg bg-white text-center focus:outline-none'>
						<div className='px-1 py-1'>
							...
							<Menu.Item>
								{({ active }) => (
									<Link
										onClick={() => fetchSubData('adventure')}
										className={classNames(
